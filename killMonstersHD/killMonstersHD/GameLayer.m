...
- (void) update: (ccTime) dt
{
    if(IsGameActive == YES)
    {
        ...
        for(CCSprite *currentBullet in bulletsArray)
        {
            if(currentBullet.position.x > GameWidth || currentBullet.position.x <  0 || currentBullet.position.y > GameHeight || currentBullet.position.y < 0)
            {
            ...
            for(Monster *currentMonster in monstersArray)
            {
                if(abs(currentBullet.position.x - currentMonster.position.x) <= 20 && abs(currentBullet.position.y - currentMonster.position.y) <= 20)
                {
...
- (void) doShot: (CGPoint) location
{
    ...
    CGPoint shotPoint = ccp((normalVector.x * 1100) + playerSprite.position.x, (normalVector.y * 1100) + playerSprite.position.y);
    ...
    [bulletSprite runAction: [CCMoveTo actionWithDuration: 2.5 position: shotPoint]];
}
